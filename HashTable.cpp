...
int HashTable::hash(string itemKey)
{
	int value = 0, a = 127;
	for (int i = 0; i < itemKey.length(); i++)
		value = (a * value + itemKey[i]) % length;
	return value;
}

// Returns an array location for a given item key.
int HashTable::hash1(string itemKey)
{
	int value = 0;
	for (int i = 0; i < itemKey.length(); i++)
		value ^= itemKey[i];
...
// Adds an item to the Hash Table.
void HashTable::insertItem1(Item * newItem)
{
	int index = hash1(newItem->key);
	array[index].insertItem(newItem);
}
...
float HashTable::countHistogram(int N)
{
	float sum = 0;
	float mean = N / length;
//	cout << "\n\nHash Table Contains ";
//	cout << getNumberOfItems() << " Items total\n";
	for (int i = 0; i < length; i++)
	{
sum+=(array[i].getLength() - mean)*(array[i].getLength() - mean);
	}
	return sum / length;
}
